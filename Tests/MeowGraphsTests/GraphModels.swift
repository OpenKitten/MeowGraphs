...
class User : Model {
	...
	internal required init(restoring source: BSON.Primitive, key: String) throws {
	...
	internal required init(newFrom source: BSON.Primitive) throws {
		guard let document = source as? BSON.Document else {
			throw Meow.Error.cannotDeserialize(type: User.self, source: source, expectedPrimitive: BSON.Document.self);
		}
	...
	internal var _id = Meow.pool.newObjectId() { didSet { Meow.pool.free(oldValue) } }
